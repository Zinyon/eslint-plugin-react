...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function handleSFCUsage(node) {
      ...
      const isPropUsed = (node.object.name === 'props' || node.object.name === 'context' || node.object.callee.name === 'useContext') && !isAssignmentLHS(node);
      if (isPropUsed && configuration === 'always') {
        context.report({
          node,
          message: `Must use destructuring ${node.object.name || node.object.callee.name} assignment`
        });
      }
    }
    ...
    return {
      ...
      VariableDeclarator(node) {
        ...
        if (SFCComponent && destructuringSFC && configuration === 'never') {
          context.report({
            node,
            message: `Must never use destructuring ${node.init.name || node.init.callee.name} assignment`
          });
        }
